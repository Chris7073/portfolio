...
<template>
  <header class="bg-gray-50/80 w-full p-4 sticky top-0 backdrop-blur-sm border-b border-gray-200/60 dark:bg-slate-900/80 dark:border-slate-500/60 shadow-sm z-[1000]">
    <div class="container mx-auto flex justify-between items-center">
      
      <NuxtLink to="/">
        <h1 class="uppercase font-semibold text-slate-800 dark:text-slate-200 tracking-wider">
          {{ websiteInfo?.name }}
        </h1>
      </NuxtLink>

      <div class="flex gap-2 items-center"> <div v-for="social of socialLinks" :key="social.id"> <NuxtLink v-if="social.active" :to="social.link" target="_blank">
            <Button 
