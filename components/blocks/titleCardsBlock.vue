...
<template>
    <section 
        ref="blockRef"
        class="group bg-slate-100 dark:bg-slate-900 py-20 sm:py-28 lg:py-32 px-6 sm:px-8"
        :class="{ 'is-visible': isVisible }"
    >
        <div class="max-w-5xl mx-auto text-center transition-all duration-1000 ease-out opacity-0 -translate-y-4 group-[.is-visible]:opacity-100 group-[.is-visible]:translate-y-0">
            <h1 
        ...
        <div v-if="blockData.cards && blockData.cards.length > 0" class="max-w-7xl mx-auto">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
                <div 
                    v-for="(card, index) in blockData.cards" 
                    :key="card.c_id" 
                ...
                >
                    <div class="flex flex-col items-start text-left h-full">
                        <div class="mb-5">
                            <span class="inline-flex justify-center items-center w-14 h-14 rounded-full bg-sky-100 dark:bg-sky-900/50">
