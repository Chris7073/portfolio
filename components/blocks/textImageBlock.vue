...
<template>
    <section 
        v-if="blockData.block_content && blockData.image_url"
        ref="blockRef"
        class="group bg-slate-50 dark:bg-slate-900"
        :class="{ 'is-visible': isVisible }"
    >
        <div class="grid grid-cols-1 lg:grid-cols-2 p-8 gap-4">
            <div 
                class="flex items-center  sm:px-10 md:py-24 lg:px-12 lg:py-32"
                :class="{ 'lg:order-last': blockData.imagePosition === 'left' }"
            >
                <div 
                    class="mx-auto max-w-xl transition-all duration-1000 ease-out group-[.is-visible]:opacity-100 group-[.is-visible]:translate-x-0"
                    :class="[
            ...
            <div 
                class="group/image relative aspect-video lg:aspect-auto overflow-hidden transition-all duration-1000 ease-out delay-200 group-[.is-visible]:opacity-100 group-[.is-visible]:scale-100"
                :class="['opacity-0 scale-95']"
            >
                <div class="relative h-full w-full rounded-lg overflow-hidden isolation-isolate">
                    <img 
                        class="block h-full w-full object-cover object-center transition-transform duration-500 ease-in-out"
                        :class="blockData.hover_effect ? 'group-hover/image:scale-105' : ''"
