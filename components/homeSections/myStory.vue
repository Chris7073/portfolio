...
<template>
  <section id="about" ref="myStory" class="relative z-20 bg-slate-100 dark:bg-slate-900 py-16 lg:py-24 px-4 overflow-hidden">
    <div v-animate-on-scroll
      ...
      <div 
        v-else 
        class="opacity-0 -translate-y-8 transition-all duration-1000 ease-out group-[.is-visible]:opacity-100 group-[.is-visible]:translate-y-0"
      >
        <div v-if="storyInfo && storyInfo.story">
          ...
          <div v-if="storyInfo.story_me && storyInfo.story_me.length > 0"
               class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
            <div v-for="(story, index) in storyInfo.story_me" :key="story.me_id || index"                  class="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-lg hover:shadow-2xl border border-transparent 
                        hover:border-sky-500/50 transition-all duration-300 ease-in-out transform hover:-translate-y-1
