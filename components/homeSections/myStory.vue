...
<template>
  <section id="about" class="relative z-20 bg-slate-100 dark:bg-slate-900 py-16 lg:py-24 px-4">
      ...
      <div v-else>
        <div v-if="storyInfo && storyInfo.story">
          ...
          <div v-if="storyInfo.story_me && storyInfo.story_me.length > 0"
            ...
            <div v-for="(story, index) in storyInfo.story_me" :key="story.me_id || index" class="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-lg hover:shadow-2xl border border-transparent hover:border-sky-500/50 transition-all duration-300 ease-in-out transform hover:-translate-y-1">
