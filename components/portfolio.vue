...
const { data: PortfolioPostsInfo } = useFetch('/api/portfolio-posts');
...
<template>
    ...
    <div id="portfolio">
        <div v-if="filteredPostsCatId?.some(item => item.post_active)" class="grid lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 p-4 gap-4">
            <div v-for="item of filteredPostsCatId" >
                <NuxtLink v-if="item.post_active" :to="'/projects/' + item.post_id">
                    <div class="cursor-pointer text-white bg-cover rounded-lg transition delay-0 duration-400 ease-in-out hover:-translate-y-1 hover:scale-101"
                        ...
                        <div class="grid h-100 content-end bg-black/10 hover:bg-black/30 transition-colors rounded-lg">
